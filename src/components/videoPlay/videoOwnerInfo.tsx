...
import { MembershipInfo } from '../../types/types';
...
const VideoOwnerInfo: React.FC<VideoOwnerInfoProps> = ({ VideoInfo }) => {
  ...
  const isOwner = VideoInfo?.video?.owner?._id === info?._id;

  useEffect(() => {
    const checkMembershipStatus = async () => {
      if (!isLoggedIn || !VideoInfo.video.owner._id) {
        return;
      }

      try {
        const channelId = VideoInfo.video.owner._id;

        const response = await getMembershipStatus(channelId);

        const { status, membership, displayPrice } = response.data.data;
    ...
    if (VideoInfo?.video?.owner?._id && isLoggedIn) {
      checkMembershipStatus();
    }
  }, [VideoInfo.video.owner._id, isLoggedIn]);

  useEffect(() => {
    console.log('watch status : ', VideoInfo.canWatch);
    if (VideoInfo.canWatch == false) {
      if (!isLoggedIn) {
  ...
  const toggleReaction = async (type: 'like' | 'dislike') => {
    try {
      const res = await toggleVideoReaction(
        VideoInfo.video._id,
        type
      );
  ...
  const handleShare = useCallback(() => {
    if (navigator.share) {
      ...
      alert(
        'Your browser does not support direct sharing. You can share via these options below.'
      );
    }
  }, [VideoInfo]);
      ...
      >
        <div className="text-center">
          ...
          <p style={{ color: theme.text }}>
            Failed to load video owner info. Please try again.
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full space-y-4 sm:space-y-6">
      ...
      >
        ...
        <div className="hidden sm:flex flex-row justify-between items-center gap-4 p-4 lg:p-6">
          ...
          <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
            ...
            <div
              onClick={() =>
                navigate(
                  `/videoPlay/ownerProfile/${VideoInfo.video.owner.username}`
                )
              }
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={isOwner}
                title={isOwner ? "You cannot join your own membership" : ""}
              >
                {isOwner ? "Owner" : membershipInfo.status === "active" ? "Member" : "Join"}
              </button>
          ...
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            >
              <div
                className="flex items-center gap-1 border-r pr-2 lg:pr-3"
                style={{ borderColor: theme.border }}
              >
                <ThumbsUp
        ...
        <div className="sm:hidden p-4 space-y-4">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <div
                className="flex items-center gap-2 border-r pr-3 flex-1"
                style={{ borderColor: theme.border }}
              >
                <ThumbsUp
      ...
      >
        <div className="p-4 lg:p-6">
          ...
          <div
            className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 border-b pb-4"
            style={{ borderColor: theme.border }}
          >
            <h1
          ...
          <p
            className="text-sm lg:text-base mt-4 leading-relaxed"
            style={{ color: theme.textSecondary }}
          >
      ...
      {VideoInfo.canWatch && <VideoComments VideoId={VideoInfo.video._id} />}
      {showLoginModal && (
        <LoginModal
          onClose={() => {
      ...
      {membershipModal && (
        <MembershipModal
          ...
          displayPrice={membershipInfo.displayPrice}
          channelName={VideoInfo.video.owner.fullName}
          channelAvatar={VideoInfo.video.owner.avatar}
          channelId={VideoInfo.video.owner._id}
