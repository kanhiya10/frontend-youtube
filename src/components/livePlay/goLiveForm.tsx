...
const GoLiveForm: React.FC = () => {
  ...
  const handleGoLive = async () => {
    try {
      const res = await axios.post(`${process.env.VITE_API_URL}/api/v1/stream/start`, { title, category }, {
  ...
  const handleStopStream = async () => {
    try {
      const res = await axios.post(`${process.env.VITE_API_URL}/api/v1/stream/stop`,{}, {
